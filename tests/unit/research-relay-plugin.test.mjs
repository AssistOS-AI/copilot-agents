...
test('researchRelay contributes launch metadata instead of a separate Explorer menu item', () => {
    ...
    assert.equal(config.label, 'Research relay provider metadata');
    assert.equal(config.copilotLaunch.query['forward-envelope'], '1');
    assert.equal(config.copilotLaunch.query['research-tags'], undefined);
    assert.equal(config.copilotLaunch.query['tag-relay-agent'], undefined);
    assert.equal(config.copilotLaunch.query['tag-relay-submit-tool'], undefined);
    assert.equal(config.copilotLaunch.query['tag-relay-list-tool'], undefined);
    assert.equal(config.copilotLaunch.query['tag-relay-tags'], undefined);
