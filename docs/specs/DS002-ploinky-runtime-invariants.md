...
Ploinky framework code must remain agent-agnostic. It must not hardcode `copilotProviderRelay`, `openInterpreterAgent`, backend ids, provider tool names, or any other optional agent id. Ploinky must not add a provider-suite enable command or treat a bundle enable command as the chat availability contract. A provider is selectable in Copilot only when the selected chat agent discovers a launcher skill for it; runtime backend health remains a launcher or relay concern.

Semantic provider execution must flow from AchillesCLI launcher skills to
...
mentions are ordinary chat text and must fall through to the selected chat
agent. Server-side relay calls must use router-mediated MCP with invocation
...
Response:
Every provider backend already needs a provider agent to own package
