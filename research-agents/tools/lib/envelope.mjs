// Tool I/O envelope helper. Copy of the shared contract in DS001.
// Ploinky mounts each agent at /code, so each agent ships its own copy
// rather than depending on a workspace-root module tree.
...
export function normalizeEnvelope(value) {
    ...
    if (input && typeof input === 'object' && !Array.isArray(input)) {
        // MCP clients sometimes nest the actual payload under input.input.
